...
!function(){"use strict";function ngTableDataService(){function getData($defer,params,api){function filterdata($defer,params){var from=(params.page()-1)*params.count(),to=params.page()*params.count(),filteredData=self.cache.result.slice(from,to);params.total(self.cache.total),$defer.resolve(filteredData)}self.cache?filterdata($defer,params):api&&api.get(function(data){self.cache=data,filterdata($defer,params)})}var self=this;this.cache=null,this.getData=getData}angular.module("app.dashboard").service("ngTableDataService",ngTableDataService)}();
